//ezeket mindig deklarálni kell ha canvassal dolgozunk
...
function newColumn() {
    ...
    columns.push(
        ...
        {
            x: canvas.width,
            y: h + GAP, //alsó oszlop innen kezdődik
            width: 30,
...
function draw(){
    ...
    ctx.fillRect(10, 10, 20, 50)
        // xPos, yPos, szélesség(jobbra), magasság(lefele) */
...
function gameLoop(now = performance.now()) {
    const dt = (now - prevTime) / 1000 //eltelt idő számítása 
    prevTime = now
...
function update(dt) {
    ...
    if(firstColumn.x <  -firstColumn.width){ //ha az egésze már kint van a canvasról
