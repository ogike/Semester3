...
public class Persistence {
    ...
    public Model load(File file) throws SudokuIOException {
        try (Scanner sc = new Scanner(file)) {
            ...
            for (int i = 0; i < Model.TABLE_SIZE; ++i) {
                for (int j = 0; j < Model.TABLE_SIZE; ++j) {
                    ...
                    if (table[i][j] <= 0) {
                        fixed[i][j] = false;
                        table[i][j] = -table[i][j];
                            //0 esetén nem változik
                            //ha negatív szám az, amit a player írt be eddig
                                //ilyenkor pozitív megjeleníthetővé tesszük
                    } else {
                        fixed[i][j] = true;
                    }
                }
            ...
            //mentett játék betöltése esetén
            int time = 0;
            if (sc.hasNextInt()) {
                time = sc.nextInt();
            }
            ...
            for (int i = 0; i < Model.TABLE_SIZE; ++i) {
                for (int j = 0; j < Model.TABLE_SIZE; ++j) {
                    ...
                    if (!model.isFixed(i, j)) {
                        num = -num;
                        //ha player írta, akkor negatívként tároljuk
                    }
                    pw.print(num + " ");
                }
                pw.println();
